...
describe('reports', () => {
  ...
  it('authorized user can create org-scoped scientific report', async () => {
    ...
    await assertSucceeds(
      (async () => {
        ...
        await setDoc(doc(db, 'organizations', ORG_A, 'reports', 'new_report'), {
  ...
  it('authorized user can archive org-scoped report via update', async () => {
    ...
    await assertSucceeds(
      (async () => {
        ...
        await updateDoc(doc(db, 'organizations', ORG_A, 'reports', 'report_a1'), {
          status: 'archived',
          updated_at: new Date().toISOString(),
        });
      })()
  ...
  it('user without write_reports cannot create org-scoped report', async () => {
    ...
    await assertFails(
      (async () => {
        const { setDoc } = await import('firebase/firestore');
        await setDoc(doc(db, 'organizations', ORG_A, 'reports', 'blocked_report'), {
          organization_id: ORG_A,
          title: 'Blocked',
          status: 'draft',
        });
